...
- [x] **Day-2 manifests** — versioned examples for Hetzner CCM, CSI, Cluster Autoscaler, System Upgrade Controller (as apply/kustomize/Helm), matching what `hetzner-k3s create` installs (**CCM + CSI + cluster-autoscaler + SUC**: `docs/k3s-day2.md`, `config/samples/complex/k3s-day2-ccm/`, `k3s-day2-csi/`, `k3s-day2-cluster-autoscaler/`, `k3s-day2-system-upgrade/`)
