...
- **`REMOTE HOST IDENTIFICATION HAS CHANGED` for the server IP:** Normal after you delete and recreate a `HCloudServer` (or Hetzner reuses an address): the VM has a new host key but your `~/.ssh/known_hosts` still has the old one. Remove the **server’s public IP** (the one in `kubectl get hcloudserver … -o jsonpath='{.status.publicIPv4}'`), not an agent IP, for example `ssh-keygen -R 91.107.x.x` and optionally `ssh-keygen -R '[91.107.x.x]:22'`. Then use `scp -o StrictHostKeyChecking=accept-new …` or confirm the new fingerprint when SSH prompts.
- **`kubectl` talks to `localhost:8080` after `export KUBECONFIG=…`:** `KUBECONFIG` must point to a file that **exists** and contains a valid cluster definition. If `scp` failed, `./k3s-hc.yaml` was never created but you still exported it, later `kubectl` calls have nothing to use (or fall back to an empty default). Unset or repoint `KUBECONFIG` to your **management** cluster (e.g. kind) before `kubectl get hcloudserver …`, fetch `SERVER_IP`, copy the file, then either `export KUBECONFIG="$PWD/k3s-hc.yaml"` or use a one-shot: `KUBECONFIG=./k3s-hc.yaml kubectl get nodes` so you do not lose the management context in the same shell.
- **Join/verify scripts while `KUBECONFIG` is the workload file:** `contrib/k3s-optional/configure-k3s-join-agents.sh` and `verify-k3s-join-cluster.sh` read `HCloudServer` via kubectl. If you keep `KUBECONFIG=./k3s-hc.yaml` exported, set **`export HKC_KUBECONFIG="$HOME/.kube/config"`** (or your kind kubeconfig path) in that shell so those scripts still reach the management cluster; or unset workload `KUBECONFIG` before running them.
- **`scp` / `ssh`: `Could not resolve hostname` with an empty host:** Usually `SERVER_IP` is empty because `kubectl get hcloudserver` ran with the wrong kubeconfig (for example `KUBECONFIG` still pointed at a missing workload file). Fix management access first, `echo "$SERVER_IP"` to confirm, then `scp`.
